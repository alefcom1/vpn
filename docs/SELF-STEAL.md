...
Донором маскировки для REALITY выступает свой же веб-сервер, а не чужой сайт.
Трафик, не прошедший аутентификацию REALITY, Xray передаёт на него как есть —
значит сканер, зонд или случайный посетитель получает **настоящий сайт
с валидным сертификатом**.

Это лучше любого чужого донора: не нужно искать сайт с подходящим TLS вне CDN,
гадать о его доступности из РФ и надеяться, что он не «затаскан» публичными
сервисами. Донор свой — он всегда доступен и всегда отвечает правдой.

Отдельный вопрос — порт. REALITY на 443 неотличим от обычного HTTPS, на 8443
он выделяется. Это решает вариант B ниже, но ценой правки боевого прокси.

## Два варианта

### A. Частичный: донор свой, порт прежний

Xray остаётся на своём порту, а `dest` указывает на локальный веб-сервер.
Наружу ничего не переезжает, продакшен не трогается вовсе.

```
интернет → :443  Caddy → сайты                (как было)
           :8443 Xray ─┬─ REALITY-клиент → VPN
                       └─ зонд/сканер → 127.0.0.1:443 Caddy → настоящий сайт
```

Что это даёт по сравнению с чужим донором:

- сертификат всегда валиден, сайт всегда доступен — донор свой;
- задержка до донора доли миллисекунды вместо десятков;
- не нужно искать домен без CDN и следить, не заблокировали ли его.

Чего не даёт: VPN по-прежнему на нестандартном порту.

**`SELFSTEAL_XVER=0` обязателен.** PROXY protocol здесь не нужен — на порт VPN
попадают только зонды, а не посетители сайтов, — и включать его нельзя: обычный
Caddy заголовка не ждёт и оборвёт соединение.

```ini
SELFSTEAL=yes
SELFSTEAL_DEST=127.0.0.1:443
SELFSTEAL_SNI=свой-домен.tld
SELFSTEAL_XVER=0
XRAY_REALITY_PORT=8443
```

Больше ничего не требуется: ни правки прокси, ни простоя, ни отката.

### B. Полный: Xray занимает 443

Всё остальное в этом документе — про него. Он снимает последний минус (порт),
но требует правки боевого прокси, PROXY protocol и минуты простоя.
Делать имеет смысл, когда вариант A уже работает.
